...
Usage::
    ...
    uv run python tests/fixtures/_generator/build_fixtures.py [OUT_DIR]

With no argument the committed fixtures under ``tests/fixtures/emails/`` are
regenerated in place. Pass a directory to write elsewhere (e.g. for a diff
check against the committed copies).
"""
...
def _add_related_image(m: EmailMessage, data: bytes, *, cid: str, filename: str) -> None:
    """Attach ``data`` as a related image on the ``text/html`` alternative of ``m``."""
...
def f1_fwd_bank_advice_pdf(out: Path) -> None:
    ...
    def line(t: str, dy: float = 6 * mm, size: int = 9) -> None:
        nonlocal y
    ...
    html = (
        ...
        "<p>Dear Sir/Madam,<br>The attached payment advice is issued at the "
        "request of our customer.</p>"
        '<img src="cid:logo001">'
        "</body></html>"
    )
    m = _mkmsg(
        ...
        (
            "FW: Payment Advice - Advice Ref [GT9KX2M4P1Q7-IN]"
            " / Priority payment / Customer Ref [NORDICAUTO26021908]"
        ),
        formatdate(1771579874),
        plain,
        html,
    )
    _add_related_image(m, TINY_PNG, cid="logo001", filename="image003.png")
    m.add_attachment(
        pdf.getvalue(),
...
def f2_fwd_body_table(out: Path) -> None:
    raw = [
        ...
        (3, "2-Feb-26", "FCI2510007042", "39.247", "1,435,655.26"),
        (4, "3-Feb-26", "FCI2510007049", "39.160", "1,432,472.80"),
        (5, "3-Feb-26", "FCI2510007064", "39.259", "1,436,094.22"),
        (6, "4-Feb-26", "FCI2510007070", "39.317", "1,438,215.86"),
        (7, "4-Feb-26", "FCI2510007081", "39.493", "1,444,653.94"),
        (8, "5-Feb-26", "FCI2510007105", "39.444", "1,442,861.52"),
        (9, "5-Feb-26", "FCI2510007097", "40.188", "1,470,077.04"),
        (10, "6-Feb-26", "FCI2510007116", "39.232", "1,435,106.56"),
        (11, "6-Feb-26", "FCI2510007117", "33.601", "1,229,124.58"),
        (12, "6-Feb-26", "FCI2510007123", "39.353", "1,439,532.74"),
        (13, "6-Feb-26", "FCI2510007124", "20.259", "741,074.22"),
    ]
    rows = [(n, d, inv, qty, "Invoice", "INR", amt, amt) for n, d, inv, qty, amt in raw]
    body_rows = "".join("<tr>" + "".join(f"<td>{c}</td>" for c in r) + "</tr>" for r in rows)
    narrative = (
        "<p>We have remitted the payment of Rs.1,36,70,691.00 vide UTR no. "
        "STBK52026021813360279 on 18.02.2026.\n"
...
def f3_fwd_multiline_pdf(out: Path) -> None:
    ...
    for page in range(2):
        ...
        def line(t: str, dy: float = 5.5 * mm, size: int = 8) -> None:
            nonlocal y
            c.setFont("Helvetica", size)
        ...
        rows_for_page = data[4:] if page else data[:8]
        for d in rows_for_page:
    ...
    html = (
        "<html><body><p>Dear Sir,</p><p>Please find attached our payment advice.</p>"
...
<tr><td></td><td>Rounded total</td><td style='text-align:right'>{total_rounded}</td></tr></table>"""
    ...
    html = (
        "<html><body><p>Dear Sir,</p>"
        "<p>Payment confirmation for extrusions product, as below.</p>"
        f"{b1}{b2}<p>Regards,<br>Sunrise Extrusions</p></body></html>"
    )
    plain = (
...
def f6_direct_excel(out: Path) -> None:
    ...
    adv_hdr = ["Amount of Advance", "TDS Amount", "Net amount Remitted", "UTR Reference"]
    for col, val in zip("BCDE", adv_hdr, strict=True):
        ws[f"{col}5"] = val
    ...
    hdr = [
        ...
        "Inv Date",
        "Inv Amount",
        "Amount of which TDS is Deducted",
        "TDS amount",
        "Amount of CN/other Adjustment",
        "Net amount Remitted",
        "UTR Reference",
    ]
    for col, val in zip("ABCDEFGH", hdr, strict=True):
        ws[f"{col}8"] = val
...
def main(argv: list[str]) -> None:
    out = Path(argv[1]) if len(argv) > 1 else Path(__file__).parent.parent / "emails"
